...
export class SelectCoin{
    ...
    isQtyFulfilled = (outputAmount: Value, cumulatedAmount: Value, 
      minUTxOValue: number, nbFreeUTxO: number) : Boolean => {
      let amount : Value = outputAmount;
    
      if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
        const minAmount : Value = this.Wasm.Value.new(
          this.Wasm.min_ada_required(
            cumulatedAmount, 
            this.Wasm.BigNum.from_str(minUTxOValue.toString())
          ));
            ...
            amount = minAmount.checked_add(
              this.Wasm.Value.new(
                this.Wasm.BigNum.from_str(
                  this.ProtocolParameters.minUtxO.toString()
                )
              )
            );
          }
        ...
        if (nbFreeUTxO > 0 && this.ProtocolParameters) {
          let maxFee : any = BigInt(this.ProtocolParameters.minFeeA) * 
          BigInt(this.ProtocolParameters.maxTxSize) + BigInt(this.ProtocolParameters.minFeeB);
      ...
      randomSelect = (utxoSelection : UtxOSelection, outputAmount : Value,
         limit : number, minUTxOValue : number) : UtxOSelection => {
      ...
      descSelect = (utxoSelection : UtxOSelection, outputAmount : Value, 
        limit : number, minUTxOValue : number):UtxOSelection=>{
        // eslint-disable-next-line no-param-reassign
    ...
    randomImprove = (inputs : UTxOList, outputs : TransactionOutputs, limit : number) : SelectionResult => {
      ...
      splitOutputsAmounts.forEach((output) => {
        ...
        const ideal : Value = this.Wasm.Value.new(
          this.Wasm.BigNum.from_str('0')).checked_add(output).checked_add(output);
        const maximum : Value = this.Wasm.Value.new(
          this.Wasm.BigNum.from_str('0')).checked_add(ideal).checked_add(output);
