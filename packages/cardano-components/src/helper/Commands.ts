...
export default class Commands {
    ...
    constructor(Cardano : any, Buffer : any, Wasm : any, AddressReturnType : object){
    ...
    getUsedAddresses = async (type? : string) : Promise<Array<BaseAddress>> => {
        ...
        if(type === CardanoAPI.AddressReturnType.bech32){
            let bechAddr : Array<BaseAddress> = [];
            addr.forEach((address : any)=>bechAddr.push(this.getBech32FromHex(address)));
            return bechAddr;
        }
        return addr;
    ...
    getUtxos = async(amount? : Value,  paginate?: {page: number , limit: number} ) : Promise<Array<TransactionUnspentOutput>> => {   
