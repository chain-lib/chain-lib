...
export default class Spend{
    ...
    send = async({address, amount = 0, assets = [], metadata = null, metadataLabel = '721'} : Send) : Promise<string> => {
    ...
    sendMultiple = async ({recipients = [], metadata = null, metadataLabel = '721'}: SendMultiple) : Promise<string> => {
        const PaymentAddress = await this.Commands.getChangeAddress(CardanoAPI.AddressReturnType.bech32)
