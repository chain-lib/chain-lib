...
import { CardanoAPI as CardanoAPIRef } from '@chain-lib/cardano-api';
const CardanoAPI = new CardanoAPIRef();
...
export const update = () => async(dispatch : any) => {
    ...
    const unusedAddresses = await CardanoAPI.Commands.getUnusedAddresses(
        bech32 ? CardanoAPIRef.AddressReturnType.bech32 : CardanoAPIRef.AddressReturnType.hex);
    const usedAddresses = await CardanoAPI.Commands.getUsedAddresses(
        bech32 ? CardanoAPIRef.AddressReturnType.bech32 : CardanoAPIRef.AddressReturnType.hex);
    const rewardAddress = await CardanoAPI.Commands.getRewardAddress(
        bech32 ? CardanoAPIRef.AddressReturnType.bech32 : CardanoAPIRef.AddressReturnType.hex);
    const changeAddress = await CardanoAPI.Commands.getChangeAddress(
        bech32 ? CardanoAPIRef.AddressReturnType.bech32 : CardanoAPIRef.AddressReturnType.hex);
...
export const send = (recipients : object, metadata? : String, metadataLabel? : String) => async(dispatch : any) => {
    const send = await CardanoAPI.Send.sendMultiple({recipients : recipients, 
